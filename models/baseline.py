...
class Seq2seq(BaseTFModel):
    def __init__(self, sess, config, api, log_dir, forward, scope=None):
        ...
        with variable_scope.variable_scope("wordEmbedding"):
            ...
            if config.sent_type == "bow":
                ...
                input_embedding, sent_size = get_bi_rnn_encode(input_embedding, fwd_sent_cell, bwd_sent_cell,
                                                               scope="sent_bi_rnn")
                output_embedding, _ = get_bi_rnn_encode(output_embedding, fwd_sent_cell, bwd_sent_cell,
            ...
            if config.sent_type != "bi_rnn":
                ...
                bwd_sent_cell = self.get_rnncell("gru", self.sent_cell_size, keep_prob=1.0, num_layer=1)
                output_embedding, _ = get_bi_rnn_encode(output_embedding, fwd_sent_cell, bwd_sent_cell,
                                                        self.output_lens, scope="sent_bi_rnn")
        ...
        with variable_scope.variable_scope("generationNetwork"):
            dec_inputs = gen_inputs = cond_embedding
        ...
        if not forward:
            ...
            self.optimize(sess, config, self.elbo, log_dir)
    ...
    def train(self, global_t, sess, train_feed, update_limit=5000):
        ...
        loss_names = ["elbo_loss", "rc_loss", "rc_peplexity"]
        while True:
            ...
            _, sum_op, elbo_loss, rc_loss, rc_ppl = sess.run([self.train_ops, self.summary_op,
    ...
    def valid(self, name, sess, valid_feed):
        ...
        while True:
            ...
            elbo_loss, rc_loss, rc_ppl = sess.run(
                [self.elbo, self.avg_rc_loss,
        ...
        avg_losses = self.print_loss(name, ["elbo_loss", "rc_loss", "rc_peplexity"],
                                     [elbo_losses, rc_losses, rc_ppls], "")
        return avg_losses[0]
